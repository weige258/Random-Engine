    ...
    private:
        void TryRebalanceTasks()
        {
            ...
            for (size_t i = 0; i < worker_cnt; ++i)
            {
                ...
                if (count > max_val) { max_val = count; max_idx = i; }
                if (count < min_val) { min_val = count; min_idx = i; }
            }

            if (max_val - min_val < kMinGap) return;
            ...
            if (mean > 0.0)
            {
                ...
                if ((std_dev / mean) < kCvThreshold) return;
            }
